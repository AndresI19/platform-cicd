...
(`restart: always`) brings a fresh one straight back. Nothing carries between jobs — the same
clean-slate property a GitHub-hosted VM has.

The entrypoint also **wipes per-run state** (`_work`, `.runner`, `.credentials`) on every start, so
each registration is pristine rather than layered on the last run's leftovers.
...
- **A pinned `RUNNER_VERSION`** in the Dockerfile — GitHub deprecates old runner versions
  server-side, and a deprecated runner connects, is told it "cannot receive messages", and exits,
  leaving jobs queued forever. Bump the ARG when GitHub moves the line.
...
| docker CLI + **buildx** | builds images through the mounted socket. buildx is mandatory: the static CLI has no legacy builder, and buildx also honors per-Dockerfile `.dockerignore` (which `fvt-traffic` needs) |
...
The build layer cache lives in **colima's daemon**, not in the runner — so destroying the runner after
every job costs no build time.

## Networking

`compose.yml` joins the `minikube` docker network as an **external** network (minikube owns it;
compose must not try to define the node container). From that network the runner reaches:
...
This is why the runner needs no kubeconfig repoint: it is on the node's network, so the address
minikube writes is simply correct.
...
socket at runtime and adds `runner` to a matching group — hard-coding it would break on a colima base
change.

## At boot

`runner/runner.service` is a systemd **user** unit, ordered `After=platform.service` (the runner joins
minikube's network, which only exists once the cluster is up; `Wants=` not `Requires=` so a slow
platform boot just makes it retry). systemd brings the compose project up at boot and down on stop;
docker's own `restart: always` handles respawning the ephemeral runner between jobs.

Like the platform's own units, it needs `loginctl enable-linger` or it will not start at boot. See
the README's *Run & connect*.
